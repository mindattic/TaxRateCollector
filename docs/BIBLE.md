...
| [`Subscriber`](TaxRateCollector.Core/Entities/Subscriber.cs) / [`SubscribedState`](TaxRateCollector.Core/Entities/SubscribedState.cs) / [`BillingRecord`](TaxRateCollector.Core/Entities/BillingRecord.cs) | Subscription + billing. |
...
| `IRateLawExtractor` → [`ClaudeRateLawExtractor`](TaxRateCollector.Infrastructure/Services/ClaudeRateLawExtractor.cs) | AI extraction of structured rate laws from raw source content, **via `MindAttic.Legion` `LegionClient`** ([Law 3](#TRC-LAW-3)); `StubRateLawExtractor` is the keyless fallback. |
